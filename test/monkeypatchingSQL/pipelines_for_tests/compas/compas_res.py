...
train_file = os.path.join(str(get_project_root()), "test", "monkeypatchingSQL", "pipelines_for_tests", "compas", "compas_train.csv")
train_data = pd.read_csv(train_file, na_values='', index_col=0)
test_file = os.path.join(str(get_project_root()), "test", "monkeypatchingSQL", "pipelines_for_tests", "compas", "compas_test.csv")
...
result = featurizer.fit_transform(train_data)

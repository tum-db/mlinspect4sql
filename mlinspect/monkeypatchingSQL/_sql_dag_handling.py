...
class SQLHistogramUpdater:
    ...
    def sql_update_backend_result(self, mapping, pipeline_container, backend_result: BackendResult,
        ...
        if backend_result.optional_second_dag_node_annotation != None or backend_result.optional_second_annotated_dfobject != None:
            print("")
        pandas_object_columns = list(mapping.get_columns_no_track(curr_sql_expr_name))
        if set(pandas_object_columns) != set(curr_sql_expr_columns):
            print("columns not equal!")

        # print(f"Pandas: {set(pandas_object_columns)}  \n  DBMS: {set(curr_sql_expr_columns)} \n ")
        ...
        for annotation in old_dat_node_annotations.keys():  # iterate in search for HistColumn inspections
            if isinstance(annotation, HistogramForColumns):  # Currently this is the only one we need to handle:
                ...
                print(f"Sensitive: {annotation.sensitive_columns}")
                if not set(curr_sql_expr_columns).intersection(set(sensitive_columns)):  # Check for empty intersection
                    if is_input_data_source:
                        # Here no tracked cols are affected => return old histogram
                        old_dat_node_annotations[annotation] = {x: {} for x in sensitive_columns}
                    else:
                        # Here no tracked cols are affected => return old histogram
                        old_dat_node_annotations[annotation] = self.current_hist.copy()
                        print("ASSIGNED!!!!")
                        print(self.current_hist)
                ...
                for sc in sensitive_columns:  # update the values based on current table.
                    ...
                    if is_input_data_source:
                        ...
                        if sc not in curr_sql_expr_columns:
                            new_dict[sc] = self.current_hist[sc].copy()
                            continue
